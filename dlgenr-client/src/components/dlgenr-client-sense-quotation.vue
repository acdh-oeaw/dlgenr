<template>
    <div class="dlgenr-client-sense-quotation">
        ...
        <dlgenr-client-sense-quotation-dictionary-link v-on:showLink="showLink" v-for="(link,index) in links" v-bind:key="index" v-bind:link="link"/>
        <dlgenr-client-sense-quotation-note v-bind:entry="entry" v-bind:position="position" v-bind:positionOfQuotation="positionOfQuotation"/>
    </div>
</template>
<script>
...
import DlGenRClientSenseQuotationDictionaryLink from './dlgenr-client-sense-quotation-dictionary-link';
import DlGenRClientSenseQuotationNote from './dlgenr-client-sense-quotation-note';
...
export default {
    ...
    components : {
        ...
        'dlgenr-client-sense-quotation-dictionary-link' : DlGenRClientSenseQuotationDictionaryLink,
        'dlgenr-client-sense-quotation-note' : DlGenRClientSenseQuotationNote
    },
    ...
    computed : {
        ...
        links(){
            ...
            if (this.$props.entry.getElementsByTagName('entry')[0].getElementsByTagName('sense')[position].getElementsByTagName('cit')[positionOfQuotation].getElementsByTagName('ref') !== undefined){
                numberOfReferences = this.$props.entry.getElementsByTagName('entry')[0].getElementsByTagName('sense')[position].getElementsByTagName('cit')[positionOfQuotation].getElementsByTagName('ref').length;
                for (let i = 0; i < numberOfReferences; i++){
                    let linkText = '';
                    let referenceEntryId = '';
                    if (this.$props.entry.getElementsByTagName('entry')[0].getElementsByTagName('sense')[position].getElementsByTagName('cit')[positionOfQuotation].getElementsByTagName('ref')[i].getAttribute('type') !== undefined){
                        if (this.$props.entry.getElementsByTagName('entry')[0].getElementsByTagName('sense')[position].getElementsByTagName('cit')[positionOfQuotation].getElementsByTagName('ref')[i].getAttribute('type') === 'entry'){
                            linkText = this.$props.entry.getElementsByTagName('entry')[0].getElementsByTagName('sense')[position].getElementsByTagName('cit')[positionOfQuotation].getElementsByTagName('ref')[i].childNodes[0].nodeValue;
                            if (this.$props.entry.getElementsByTagName('entry')[0].getElementsByTagName('sense')[position].getElementsByTagName('cit')[positionOfQuotation].getElementsByTagName('ref')[i].getAttribute('target') !== undefined){
                                referenceEntryId = this.$props.entry.getElementsByTagName('entry')[0].getElementsByTagName('sense')[position].getElementsByTagName('cit')[positionOfQuotation].getElementsByTagName('ref')[i].getAttribute('target');

                                links.push({ 'linkText' : linkText, 'reference' : referenceEntryId});
                            }
                        }
                    }
                }
